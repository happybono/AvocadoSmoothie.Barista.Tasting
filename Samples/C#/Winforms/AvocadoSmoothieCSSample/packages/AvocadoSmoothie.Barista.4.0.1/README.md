...
- MiddleMedian vs AllMedian :
  ...
  - AllMedian applies smoothing across the full series and honors `BoundaryMode` when synthesizing samples near the edges :
    ...
    - Adaptive : slide a full window inside [0 … N − 1] (no synthetic samples).
...
- ComputeMedians
  ...
  - Boundary handling :
    ...
    - useMiddle = False (AllMedian) : boundaryMode is applied:
      ...
      - Adaptive : a full window W slides inside [0 … N − 1] (no synthetic samples).
  ...
  - Implementation :
    ...
    - Near edges: MiddleMedian clamps indices; AllMedian uses the configured boundaryMode (or Adaptive slide).
